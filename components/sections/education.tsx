              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-6 py-5 border-b border-border/50">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-primary text-xs font-mono mt-0.5">{edu.institution}</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {edu.achievements.map((achievement) => (
                      <div key={achievement} className="flex items-start gap-2.5">
                        ...
                        <span className="text-secondary-foreground text-xs">{achievement}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {trainingCourses.map((course) => (
                ...
                >
                  <span className="text-foreground text-xs font-medium">{course.title}</span>
                  <span className="text-muted-foreground text-[11px] font-mono">{course.institution}</span>
                </div>
              ))}
            </div>
          </div>
...
function GraduationCapIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M12 14l6.16-3.422A12.083 12.083 0 0120 17.75V17a2 2 0 00-2-2H6a2 2 0 00-2 2v.75a12.083 12.083 0 011.84-7.172L12 14z" />
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M12 14v7M5 9.5V17" />
    </svg>
  );
}
